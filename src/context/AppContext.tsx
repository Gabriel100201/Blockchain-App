...
interface AppContextType {
  ...
  requestTutoring: (session: Omit<TutoringSession, 'id' | 'status'>) => Promise<void>;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const connectWallet = async () => {
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Error al conectar wallet' });
    } finally {
  ...
  const assignTokens = async (amount: number) => {
    ...
    try {
      ...
      const newBalance = await mockBackend.assignTokens(state.wallet.address, amount);
      dispatch({ type: 'UPDATE_BALANCE', payload: newBalance });
    } catch (error) {
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Error al asignar tokens' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
  ...
  const loadTutors = async () => {
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Error al cargar tutores' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
  ...
  const loadTutoringHistory = async () => {
    ...
    try {
      ...
      const history = await mockBackend.getTutoringHistory(state.wallet.address);
      dispatch({ type: 'SET_TUTORING_HISTORY', payload: history });
    } catch (error) {
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Error al cargar historial' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
  ...
  const requestTutoring = async (session: Omit<TutoringSession, 'id' | 'status'>) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      dispatch({ type: 'SET_ERROR', payload: null });
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'Error al solicitar tutoría' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
